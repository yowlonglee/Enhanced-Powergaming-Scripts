...
This mod is a collection of nine player ai scripts designed for use in Baldur's Gate II: Enhanced Edition and the Enhanced Edition Trilogy. The nine scripts included are: Archers, BasicAI (for barebones play), Cleric/Ranger (to include other divine fighter multi/duals), F/M/C multi (also works with C/M), F/M multi/duals, Inquisitor, Monk, Skald, and Sorcerer. With these scripts, you can focus on the fighting aspect of combat while these take care of the nuances in the background. 

These scripts require the mod Sword Coast Stratagems (SCS) for use. These are tailored to work with the following installs: a standard SCS install, SCS + Icewind Dale Spells (SCS Component), and SCS + Spell Revisions (Revised).
## Who Am I
