# Enhanced_Powergaming_Scripts
These scripts are awesome and will make the player's (your) life easier by handling the majority of micromanagment in and out of battles. There are now nine scripts included for: Archers, BasicAI (for barebones play), Cleric/Ranger (to include other divine fighter multi/duals), F/M/C multi (also works with C/M), F/M multi/duals, Inquisitor, Monk, Skald, and Sorcerer. With these scripts, you can focus on the fighting aspect of combat while these take care of the nuances in the background. I subscribe to the playstyle of getting up some defenses, cranking up the APR, and unleashing overwhelming violence of action on the enemy. These help with the first two, and its the player's responsibility to do the rest. These were written for modded games running SCS (to include IWD spells included in SCS) and/or Item Revisions Revised and Spell Revisions Revised. No scripts, to my knowledge, offers this to players with these mods.
## Who Am I
